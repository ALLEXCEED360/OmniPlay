...
export function PlatformReport({ rows }: { rows: PlatformReportRow[] }) {
  ...
  return (
    // One platform takes the whole width; a card half as wide as its
    // panel, beside nothing, truncated its own dates. Three go abreast
    // only where three fit.
    <div
      className={
        rows.length > 2
